...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IFlat, IAreaMeasurable
    {
        ...
        public Vector3D GetNormal()
        {
            Vector3D normal = Vector3D.CrossProduct(
                this.vertices[0] + new Vector3D(this.vertices[0].X + Radius, this.vertices[0].Y, this.vertices[0].Z),
                this.vertices[0] + new Vector3D(this.vertices[0].X, this.vertices[0].Y + Radius, this.vertices[0].Z));
            normal.Normalize();
            return normal;
        }
    }
    ...
    public class Cilinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        public Cilinder(Vector3D bot, Vector3D top, double radius)
            : base(bot, top)
        {
            this.Radius = radius;
        }
        ...
        public double GetArea()
        {
            double basePerimeter = this.Radius * 2 * Math.PI;

            return GetBaseArea() * 2 + basePerimeter * GetHeight();
        }

        private double GetHeight()
        {
            return (this.vertices[0].Z - this.vertices[1].Z);
        }
    ...
    public class AdvancedFigureController : FigureController
    {
                ...
                case "cilinder":
                    {
                        ...
                        this.currentFigure = new Cilinder(bot, top, radius);
                        break;
                    }
            }
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "normal":
                    {
                        IFlat figure = this.currentFigure as IFlat;
                        if (figure == null)
                        {
                        ...
                        {
                            Console.WriteLine("{0}", figure.GetNormal());
                        }
                        break;
                    }
                case "area":
                    {
                        IAreaMeasurable figure = this.currentFigure as IAreaMeasurable;
                        if (figure == null)
                        {
                            Console.WriteLine("undefined");
                        }
                ...
                case "volume":
                    {
                        IVolumeMeasurable figure = this.currentFigure as IVolumeMeasurable;
                        if (figure == null)
                        {
                            Console.WriteLine("undefined");
                        }
